...
const { upgradeWebSocket, websocket } = (0, bun_1.createBunWebSocket)();
...
const adminMiddleware = async (c, next) => {
    ...
    const role = ((user === null || user === void 0 ? void 0 : user.role) || '').toLowerCase();
...
app.post('/api/logout', (c) => {
    (0, cookie_1.deleteCookie)(c, 'auth_token', { path: '/' });
...
app.post('/api/orders', authMiddleware, async (c) => {
    ...
    const { id, items } = body, orderData = __rest(body
    ...
    if (!waiterId && orderData.waiter) {
        ...
        waiterId = (waiterUser === null || waiterUser === void 0 ? void 0 : waiterUser.id) || 'unknown';
    }
    if (id) {
        ...
        if (existing) {
            ...
            const updated = await prisma.order.update({
                ...
                data: {
                    ...
                    pax: orderData.pax ? parseInt(orderData.pax, 10) : null,
                    ...
                    items: (items === null || items === void 0 ? void 0 : items.length) ? { create: items.map((it) => ({ name: it.name, quantity: it.quantity || it.qty || 1, rate: it.rate || 0, amount: it.amount || 0 })) } : undefined,
                },
                include: { items: true }
            });
...
app.get('/api/invoice-counter', authMiddleware, async (c) => {
    ...
    return c.json({ counter: (counter === null || counter === void 0 ? void 0 : counter.counter) || 0 });
});
...
app.post('/api/menu-items/bulk', authMiddleware, async (c) => {
    ...
    if (items.length > 0) {
        await prisma.menuItem.createMany({
            data: items.map((it) => ({ name: it.name, rate: Number(it.rate) || 0, category: it.category || null }))
        });
    }
...
app.get('/', (c) => c.html(html_1.html));
app.get('/index.html', (c) => c.html(html_1.html));
